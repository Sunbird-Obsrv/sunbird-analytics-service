...
object EmbeddedPostgresql {
  ...
  def createTables(): Boolean = {
    ...
    val query7 = "CREATE TABLE IF NOT EXISTS dataset_metadata(dataset_id VARCHAR(50), dataset_sub_id VARCHAR(50), dataset_config json, visibility VARCHAR(50), dataset_type VARCHAR(50), version VARCHAR(10), authorized_roles text[], available_from TIMESTAMP, sample_request VARCHAR(300), sample_response VARCHAR(500), validation_json json, druid_query json, limits json, supported_formats VARCHAR(20), exhaust_type VARCHAR(50), PRIMARY KEY (dataset_id));"

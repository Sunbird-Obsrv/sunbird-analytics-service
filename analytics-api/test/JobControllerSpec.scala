...
class JobControllerSpec extends FlatSpec with Matchers with BeforeAndAfterAll with MockitoSugar {
  ...
  "JobController" should "test get job API " in {
    ...
    when(restUtilMock.get[Response]("https://dev.sunbirded.org/api/user/v2/read/testUser", Option(Map("x-authenticated-user-token" -> "testUserToken")))).thenReturn(JSONUtils.deserialize[Response](response5))
    result = controller.getJob("client1", "request1").apply(FakeRequest().withHeaders(("X-Channel-ID", "testChannel")).withHeaders(("X-User-ID", "testUser")).withHeaders(("x-authenticated-user-token", "testUserToken")).withHeaders(("Authorization", "testBearerToken")))
    Helpers.status(result) should be (Helpers.FORBIDDEN)
  ...
  it should "test get telemetry API" in {
    ...
    when(restUtilMock.get[Response]("https://dev.sunbirded.org/api/user/v2/read/testUser", Option(Map("x-authenticated-user-token" -> "testUserToken")))).thenReturn(JSONUtils.deserialize[Response](response1))
    when(cacheUtil.getSuperAdminChannel()).thenReturn("channel-mhrd")
    ...
    when(restUtilMock.get[Response]("https://dev.sunbirded.org/api/user/v2/read/testUser", Option(Map("x-authenticated-user-token" -> "testUserToken")))).thenReturn(JSONUtils.deserialize[Response](response3))
    when(cacheUtil.getSuperAdminChannel()).thenReturn("channel-mhrd")
    result = controller.getTelemetry("raw").apply(FakeRequest().withHeaders(("X-Channel-ID", "channel-1")).withHeaders(("X-User-ID", "testUser")).withHeaders(("x-authenticated-user-token", "testUserToken")).withHeaders(("Authorization", "testBearerToken")))
    Helpers.status(result) should be (Helpers.OK)

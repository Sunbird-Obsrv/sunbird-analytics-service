...
class TestAccessTokenValidator extends FlatSpec with Matchers with MockitoSugar {
    ...
    def getPublicKey(keyId: String): PublicKey = {
        var publicKey = new String(keyId.getBytes(), StandardCharsets.UTF_8);
        publicKey = publicKey.replaceAll("(-+BEGIN PUBLIC KEY-+)", "");
        publicKey = publicKey.replaceAll("(-+END PUBLIC KEY-+)", "");
        publicKey = publicKey.replaceAll("[\\r\\n]+", "");
        val keyBytes = Base64.getMimeDecoder.decode(publicKey.getBytes(StandardCharsets.UTF_8)) //Base64UtilJava.decode(publicKey.getBytes("UTF-8"), Base64UtilJava.DEFAULT)

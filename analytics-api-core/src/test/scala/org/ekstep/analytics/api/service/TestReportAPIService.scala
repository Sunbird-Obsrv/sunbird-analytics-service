...
class TestReportAPIService extends FlatSpec with Matchers with BeforeAndAfterAll with BeforeAndAfterEach with MockitoSugar  {
  
  implicit val mockFc = mock[FrameworkContext];
  implicit val config = ConfigFactory.load()
  private implicit val system: ActorSystem = ActorSystem("test-actor-system", config)
  val reportApiServiceActorRef = TestActorRef(new ReportAPIService)
  implicit val executionContext: ExecutionContextExecutor =  scala.concurrent.ExecutionContext.global
  implicit val timeout: Timeout = 20.seconds


  "ReportAPIService" should "return response for data request" in {
    ...
    val response = ReportAPIService.submitReport(request)
